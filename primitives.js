/**
 * @preserve Basic Primitives orgDiagram v2.0.10
...
/*
    Function: primitives.common.compareArrays
        Compare non-object non-sorted arrays.
    ...
    switch (this.shapeType) {
        ...
        default:
            ...
            switch (this.shapeType) {
                ...
                case 1/*primitives.common.ShapeType.Oval*/: //ignore jslint
					...
					switch (treeItem.actualVisibility) {
					    ...
					    default:
					        if (this._debug) {
					            ...
					            segments = markers[itemTitleColor];
					            segments.push(new primitives.common.DotSegment(x - 1, y - 1, 2, 2, 1));
					        }
					        break;
					}
				});//ignore jslint
                ...
                if (annotationConfig.items != null && annotationConfig.items.length > 0) {
                    ...
                    if (items.length > 1) {
                        for (index2 = 1, len2 = items.length; index2 < len2; index2 += 1) {
                            ...
                            path = primitives.common.getShortestPath(graph, firstItem, nextItem);
	...
	if (this._treeItems.hasOwnProperty(rootItemId)) {
		...
		for (index = 0, len = polylines.length; index < len; index += 1) {
		    ...
		    if (polyline.segments.length > 0) {
		        ...
		        //if (this._debug && attr.borderColor != "black") {
...
primitives.orgdiagram.BaseController.prototype._redrawConnector = function (hasGraphics, buffer, parentTreeItem, parentTreeLevel) {
	...
	if (parentTreeItem.partnerConnectorOffset > 0) {
	    ...
	    if(parentTreeItem.connectorPlacement & 4/*primitives.common.SideFlag.Bottom*/) {
	        ...
	        partnersConnectorOffset = parentTreeLevel.connectorShift;
	    }
	}
    ...
    */
	...
	if (parentTreeItem.partnerConnectorOffset <= 1) {
	    ...
	    if (parentTreeItem.partnerConnectorOffset > 1) {
	        ...
	        buffer.selectPalette(parentTreeLevel.partnerConnectorOffset);
	    }
	}
	...
	if (parentTreeItem.partners.length > 1 || parentTreeItem.extraPartners.length > 0) {
	    ...
	    if (points.length > 0) {
	        ...
	        this._drawTopConnectors(buffer, parentHorizontalCenter, partnersConnectorOffset, points, true, hasGraphics);
	    }
	}
	...
	if (parentTreeItem.visualChildren.length > 0) {
		...
		if (!hideConnectors && points.length > 0) {
		    connectorStyleType = this._drawTopConnectors(buffer, parentHorizontalCenter, parentTreeLevel.connectorShift, points, hasSquared, hasGraphics);
		}
		...
		if (!hideConnectors) {
		    this.transform.transformPoints(parentHorizontalCenter, partnersConnectorOffset,
				parentHorizontalCenter, parentTreeLevel.connectorShift, true, this, function (fromX, fromY, toX, toY) {
				    ...
				    if (this.showElbowDots && this.options.elbowType != 0/*primitives.common.ElbowType.None*/ && parentTreeItem.partners.length > 0) {
				        segments.push(new primitives.common.DotSegment(fromX - 1, fromY - 1, 2, 2, 1));
				    }
				    segments.push(new primitives.common.MoveSegment(fromX, fromY));
				    segments.push(new primitives.common.LineSegment(toX, toY));
				    if (this.showElbowDots && this.options.elbowType != 0/*primitives.common.ElbowType.None*/ && points.length > 1) {
				        segments.push(new primitives.common.DotSegment(toX - 1, toY - 1, 2, 2, 1));
				    }
				});
		}
	}
...
primitives.orgdiagram.BaseController.prototype._drawTopConnectors = function (buffer, parentX, parentY, points, hasSquared, hasGraphics) {
    ...
    if (hasSquared) {
        ...
        for (index = startIndex; index <= endIndex; index+= 1) {
            ...
            if (hasGraphics) {
                switch (this.options.elbowType) {
                    ...
                    case 3/*primitives.common.ElbowType.Round*/:
                        if (Math.abs(parentX - connectorPoint.x) > bevelSize && Math.abs(parentY - connectorPoint.y) > bevelSize) {
            ...
            if (connectorPoint.hasElbow) {
                this.transform.transformPoints(connectorPoint.x, connectorPoint.y, connectorPoint.elbowPoint1.x, connectorPoint.elbowPoint1.y, true, this, function (fromX, fromY, toX, toY) {
                    segments.push(new primitives.common.MoveSegment(fromX, fromY));
                    segments.push(new primitives.common.LineSegment(toX, toY));
                });//ignore jslint

                this.transform.transform3Points(connectorPoint.elbowPoint1.x, connectorPoint.elbowPoint1.y, connectorPoint.elbowPoint1.x, connectorPoint.elbowPoint2.y, connectorPoint.elbowPoint2.x, connectorPoint.elbowPoint2.y, true, this, function (fromX, fromY, toX, toY, toX2, toY2) {
                    switch (this.options.elbowType) {
                        case 2/*primitives.common.ElbowType.Bevel*/:
                            segments.push(new primitives.common.MoveSegment(fromX, fromY));
                            segments.push(new primitives.common.LineSegment(toX2, toY2));
                            break;
                        case 3/*primitives.common.ElbowType.Round*/:
                            segments.push(new primitives.common.MoveSegment(fromX, fromY));
                            segments.push(new primitives.common.CubicArcSegment(fromX, fromY, toX, toY, toX2, toY2));
                            break;
                    }
                });//ignore jslint
            } else {
...
primitives.orgdiagram.BaseController.prototype._recalcLevelsDepth = function () {
    ...
    for (index = 0, len = this._treeLevels.length; index < len; index += 1) {
        ...
        if (treeItemsHavingPartners.length > 0) {
            ...
            treeLevel.partnerConnectorOffset = this._setConnectorsOffset(treeItemsHavingPartners);
        }
    }
};

primitives.orgdiagram.BaseController.prototype._setConnectorsOffset = function (treeItemsGroups) {
    var result = 0,
        maxOffset,
        prevOffset,
        treeItemsGroup,
    ...
    for (index2 = 0, len2 = treeItemsGroups.length; index2 < len2; index2 += 1) {
        ...
        treeItemsGroup.treeItem.partnerConnectorOffset = startPoint.offset;
...
primitives.orgdiagram.BaseController.prototype._createVisualTree = function (logicalParentItem) {
    var rootItem,
        index, len,
        orgItem, treeItem,
        key, childTreeItem, notFound, childKey;
    ...
    if (this._orgItemRoots.length > 0) {
        ...
        this._showCursorNeighbours();

        /* test consistency of references in org chart tree */
        if (this._debug) {
            for (key in this._treeItems) {
                if (this._treeItems.hasOwnProperty(key)) {
                    treeItem = this._treeItems[key];
                    for (index = 0, len=treeItem.logicalChildren.length; index < len; index += 1) {
                        childKey = treeItem.logicalChildren[index];

                        childTreeItem = this._treeItems[childKey];
                        notFound = true;
                        jQuery.each(childTreeItem.logicalParents, function (index, value) {
                            if (value == key) {
                                notFound = false;
                                return false;
                            }
...
primitives.orgdiagram.BaseController.prototype._showCursorNeighbours = function () {
...
primitives.common.SharedReferences = function (arg0) {
    this.items = [];
    this.weight = 0;
    this.difference = 0;
...
/*
    Function: primitives.common.getGraphSpanningTree
        Get maximum spanning tree. Graph may have disconnected sub graphs, so start node is nessasary.
    
    Parameters:
	graph - Hash of hashes. Where Hash key is node name and value is connection object.
    startNode - The node to start searching for maximum spanning tree.
    compareFunc - Call back function to compare two edges of graph. function(edge1, edge2)

    Returns: 
	    Hash of arrays. Where key is node name and array contains collection of children in spanning tree.
*/
primitives.common.getGraphSpanningTree = function (graph, startNode, compareFunc) {
    /* Graph */
...
/*
    Function: primitives.common.getShortestPath
    ...
    Parameters:
	graph - Hash of hashes. Where Hash key is node name.
...
*/
primitives.common.getShortestPath = function (graph, startNode, endNode) {
    var margin = {},
        distance = {},
        breadcramps = {},
        marginLength = 0,
...
/*
    Function: primitives.common.invertReferences
        Invert references. It takes hash of item references and inverts it, so target items become keys and source items become references.
    
    Parameters:
	items - Hash of arrays. Where Hash key is item name and array contains names of referenced items.
    getReferences - call back function returning collection of referenced items by item. If it is null then hash value is considered as array of references.

    Returns: 
	    Hash of arrays, where hash key is referenced names and value is array of sources names.
*/
primitives.common.invertReferences = function (items, getReferences) {
    var result = {},
        key, item, targets, targetItem,
        index, len;
    for (key in items) {
        if (items.hasOwnProperty(key)) {
            item = items[key];
            targets = getReferences != null ? getReferences(item) : item;
            for (index = 0, len = targets.length; index < len; index += 1) {
                targetItem = targets[index];
                if (!result.hasOwnProperty(targetItem)) {
                    result[targetItem] = [];
                }
                result[targetItem].push(key);
            }
        }
    }
...
/*
    Function: primitives.common.optimizeReferences
        Optimize references. It takes hash of item references and groups common references into shared items.
    
    Parameters:
	items - Hash of arrays. Where Hash key is item name and array contains names of referenced items.
    addNewReferenceItem - call back function returning unique id of new reference item..

    Returns: 
	    Hash of arrays. It is optimized items hash having regoruped shared references.
*/
primitives.common.optimizeReferences = function (items, addNewReferenceItem) {
    var results = {},
        resultItems = [],
        sharedItems = {},
        currentItems = {},
        key, targets, newNode,
        nextItems, layerGraph, node,
        index, len,
        maximumTree, treeKey, children, child,
        relation, nextSharedItem,
        item;
    for (key in items) {
        if (items.hasOwnProperty(key)) {
            targets = items[key];
            newNode = new primitives.common.ReferenceItem();
            targets.sort();
            newNode.key = targets.join(",");
            newNode.name = key;
            newNode.targets = targets.slice(0);
            newNode.actualTargets = targets.slice(0);
            newNode.level = 0;
            currentItems[newNode.name] = newNode;
        }
    }
    ...
    while (!primitives.common.isEmptyObject(currentItems)) {
        ...
        layerGraph = primitives.common.getReferencesGraph(currentItems);

        for (key in currentItems) {
            if (currentItems.hasOwnProperty(key)) {
                node = currentItems[key];

                resultItems.push(node);

                if (!node.processed) {

                    maximumTree = primitives.common.getGraphSpanningTree(layerGraph, key, function (a, b) {
                    ...
                    for (treeKey in maximumTree) {
                        if (maximumTree.hasOwnProperty(treeKey)) {
                            currentItems[treeKey].processed = true;

                            children = maximumTree[treeKey];
                            for (index = 0, len = children.length; index < len; index += 1) {
                                child = children[index];
                                currentItems[child].processed = true;

                                relation = layerGraph[treeKey][child];

                                if (relation.weight > 1) {
                                    key = relation.items.join(',');
                                    nextSharedItem = null;
                                    if (!sharedItems.hasOwnProperty(key)) {
                                        nextSharedItem = new primitives.common.ReferenceItem();
                                        nextSharedItem.name = addNewReferenceItem();
                                        nextSharedItem.key = key;
                                        nextSharedItem.actualTargets = relation.items.slice(0);
                                        nextSharedItem.targets = relation.items.slice(0);

                                        sharedItems[nextSharedItem.key] = nextSharedItem;
                                        nextItems[nextSharedItem.name] = nextSharedItem;
                                    } else {
                                        nextSharedItem = sharedItems[key];
                                    }

                                    primitives.common.addGroup(currentItems[treeKey], nextSharedItem);
                                    primitives.common.addGroup(currentItems[child], nextSharedItem);
                                }
                            }
                        }
                    }
    ...
    for (index = 0, len = resultItems.length; index < len; index += 1) {
        item = resultItems[index];
        results[item.name] = item.actualTargets;
    }

    return results;
};

primitives.common.addGroup = function (node, group) {
    var index, len,
        nodesToRemove, newTargets;
    if (node.name != group.name) {
        nodesToRemove = {};
        for (index = 0, len = group.targets.length; index < len; index += 1) {
            nodesToRemove[group.targets[index]] = true;
        }
        newTargets = [];
        for (index = 0, len = node.actualTargets.length; index < len; index += 1) {
            if (!nodesToRemove.hasOwnProperty(node.actualTargets[index])) {
                newTargets.push(node.actualTargets[index]);
            }
        }
        if (newTargets.length != node.actualTargets.length) {
            newTargets.push(group.name);
            node.actualTargets = newTargets;
        }
    }
};

primitives.common.getReferencesGraph = function (references) {
    var result = {},
        targets = primitives.common.invertReferences(references, function (item) { return item.targets; }),
        key, items, index1, index2, len, from, fromNode, to, toNode, difference;

    for (key in targets) {
        if (targets.hasOwnProperty(key)) {
            items = targets[key];
            for (index1 = 0, len = items.length; index1 < len - 1; index1 += 1) {
                from = items[index1];
                fromNode = references[from];
                for (index2 = index1 + 1; index2 < len; index2 += 1) {
                    to = items[index2];
                    toNode = references[to];
                    difference = Math.abs(fromNode.targets.length - toNode.targets.length);
                    if (!result.hasOwnProperty(from)) {
                        result[from] = {};
                    }
                    if (!result[from].hasOwnProperty(to)) {
                        result[from][to] = new primitives.common.SharedReferences(difference);
                    }
                    result[from][to].items.push(key);
                    result[from][to].weight += 1;

                    if (!result.hasOwnProperty(to)) {
                        result[to] = {};
                    }
                    if (!result[to].hasOwnProperty(from)) {
                        result[to][from] = new primitives.common.SharedReferences(difference);
                    }
                    result[to][from].items.push(key);
                    result[to][from].weight += 1;
...
/*
    Function: primitives.common.topologicalSort
        Toppological sorting of references. It takes hash of item references and sort them topologically.
    
    Parameters:
	items - Hash of arrays. Where Hash key is item name and array contains names of referenced items.

    Returns: 
	    Array of sroted references. 
*/
primitives.common.topologicalSort = function (nodes) {
    var result = [],
        references = {},
        key, targets, target, index, len, neighbour,
        queue, newQueue, node, subIndex, subLen;
    for (key in nodes) {
        if (nodes.hasOwnProperty(key)) {
            targets = nodes[key];
            for (index = 0, len = targets.length; index < len; index += 1) {
                neighbour = targets[index].toString();

                if (!references.hasOwnProperty(neighbour)) {
                    references[neighbour] = 0;
                }
                references[neighbour] += 1;
            }
        }
    }
    ...
    while (queue.length > 0) {
        ...
        for (index = 0, len = queue.length; index < len; index += 1) {
            ...
            targets = nodes[node];
            for (subIndex = 0, subLen = targets.length; subIndex < subLen; subIndex += 1) {
                target = targets[subIndex].toString();
                references[target] -= 1;
                if (references[target] == 0) {
...
/*
    Function: primitives.common.getGraphGrowthSequence
        Get graph growth sequence. The sequence of spanning tree growth. Graph may have disconnected sub graphs, so start node is nessasary.
    
    Parameters:
	graph - Hash of hashes. Where Hash key is node name and value is connection object.
    startNode - The node to start searching for maximum spanning tree.
    getWeightFunc - Call back function to weight edge of graph. function(edge)

    Returns: 
	    Array of items. 
*/
primitives.common.getGraphGrowthSequence = function (graph, startNode, getWeightFunc) {
    /* Graph */
    var result = [startNode], /* [] array of id-s */
        margin = {}, marginKey,
        itemsToRemove = [], /* if margin item has no neighbours to expand we remove it from margin*/
        hasNeighbours,
            ...
            if (margin.hasOwnProperty(marginKey)) {
                ...
                for (neighbourKey in neighbours) {
                    if (neighbours.hasOwnProperty(neighbourKey) && !processed.hasOwnProperty(neighbourKey)) {
                        neighbourWeight = getWeightFunc(neighbours[neighbourKey]);
                        hasNeighbours = true;
...
/*  Her is a list of major steps family chart makes in order to create organizational chart hiararchy 
    ...
    3. Topologically sort family items and assign levels.
...
primitives.famdiagram.Controller.prototype._createOrgTree = function () {
    var index, len,
        ...
        parents, key,
        itemsToOptimize, optimizedItems,
        child, childFamItem,
        self = this;
    ...
    if (this.options.items.length > 0) {
        ...
        /* populate logical children of family items, remove duplicates & exclude references to non-existing nodes  */
        for (itemId in this._famItems) {
            if (this._famItems.hasOwnProperty(itemId)) {
                famItem = this._famItems[itemId];
                duplicates = {};
                parents = famItem.itemConfig.parents;
                if (parents != null) {
                    for (index = 0, len = parents.length; index < len; index += 1) {
                        childItemId = parents[index];
                        if (this._famItems.hasOwnProperty(childItemId)
                            && childItemId != famItem.id
                            && !duplicates.hasOwnProperty(childItemId)) {
                            famItem.parents.push(childItemId);
                            this._famItems[childItemId].logicalChildren.push(famItem.id);
                            this._famItems[childItemId].children.push(famItem.id);
        ...
        this._sortItemsBylevels2(this._famItems);
        for (key in this._famItems) {
            if (this._famItems.hasOwnProperty(key)) {
                famItem = this._famItems[key];
                famItem.level2 = famItem.level;
                famItem.level = null;
            }
        }
        ...
        for (key in this._famItems) {
            if (this._famItems.hasOwnProperty(key)) {
                famItem = this._famItems[key];
                famItem.logicalChildren.sort(function (a, b) { return self._famItems[a].level2 - self._famItems[b].level2; });//ignore jslint
                famItem.logicalParents.sort(function (a, b) { return self._famItems[b].level2 - self._famItems[a].level2; });//ignore jslint
            }
        }

        /* Distribute this._famItems by levels. */
        this._sortItemsBylevels(this._famItems);

        /* regroup items, bundle connectors where it is possible */
        itemsToOptimize = {};
        for (key in this._famItems) {
            if (this._famItems.hasOwnProperty(key)) {
                itemsToOptimize[key] = this._famItems[key].logicalChildren.slice(0);
            }
        }

        optimizedItems = primitives.common.optimizeReferences(itemsToOptimize, function () {
            var result;
            ...
            result.id = self.maximumFamItemId;
            self._famItems[result.id] = result;

            return result.id;
        });

        for (key in optimizedItems) {
            if (optimizedItems.hasOwnProperty(key)) {
                children = optimizedItems[key];
                self._famItems[key].logicalChildren = children;
            }
        }

        /* eliminate many to many connections in chart */
        this._eliminateManyToMany(this._famItems,
            function () {
                var result;
                ...
                result = new primitives.famdiagram.FamilyItem();
                result.isVisible = false;
                result.itemConfig = new primitives.famdiagram.ItemConfig({ id: self.maximumFamItemId });
                result.id = self.maximumFamItemId;
                return result;
            }
        );
        ...
        this._fillInItems(this._famItems,
            function () {
                var result;
        ...
        if (this._debug) {
            for (key in this._famItems) {
                if (this._famItems.hasOwnProperty(key)) {
                    famItem = this._famItems[key];
                    children = famItem.logicalChildren;

                    for (index = 0; index < children.length; index += 1) {
                        child = children[index];

                        childFamItem = this._famItems[child];
                        if (childFamItem.logicalParents.indexOf(key) < 0) {
                            throw "Family tree is broken. Logical child does not reference logical parent.";
                        }

                        if (childFamItem.level === null || famItem.level === null || childFamItem.level != famItem.level + 1) {
        ...
        /* create collection of non-extracted items */
        this._famItemsNonExtracted = {};
        for (key in this._famItems) {
            if (this._famItems.hasOwnProperty(key)) {
                this._famItemsNonExtracted[key] = this._famItems[key];
            }
        }
            ...
            while (sortedFamilies.length < this._families.length) {
                for (index = 0, len = this._families.length; index < len; index += 1) {
                    ...
                    if (!sortedFamiliesHash.hasOwnProperty(family.id)) {
                        ...
                        spanningTree = primitives.common.getGraphSpanningTree(familiesGraph, family.id, function (a, b) {
                            return a - b;
                        }); //ignore jslint
        ...
        this._balanceChildrenForItem(extraGravities, grandChildren, this._orgItemRoots[0]);
    }
};
...
primitives.famdiagram.Controller.prototype._getGrandChildren = function () {
    var itemId, orgItem,
        ...
        result = {};  /* Key = primitives.orgdiagram.OrgItem.id, Value= Hash {} having Key = level and Value = number of grand children*/
    ...
    for (levelIndex = this.maximumLevel; levelIndex >= this.minimumLevel; levelIndex -= 1) {
        ...
        if (levelItems != null) {
            for (index = 0, len = levelItems.length; index < len; index += 1) {
                ...
                this._sumGrandChildren(result, orgItem);
            }
        }
    }
...
primitives.famdiagram.Controller.prototype._sumGrandChildren = function (grandChildren, orgItem) {
    var parentId = orgItem.parent,
        level, itemId;
    if (parentId != null) {
        if (!grandChildren[parentId]) {
        ...
        if (!grandChildren[parentId][level]) {
            grandChildren[parentId][level] = 1;
        } else {
            grandChildren[parentId][level] += 1;
        }

        itemId = orgItem.id;
        if (grandChildren[itemId] != null) {
            for (level in grandChildren[itemId]) {
                if (grandChildren[itemId].hasOwnProperty(level)) {
                    if (!grandChildren[parentId][level]) {
                        grandChildren[parentId][level] = grandChildren[itemId][level];
                    } else {
                        grandChildren[parentId][level] += grandChildren[itemId][level];
                    }
                }
...
primitives.famdiagram.Controller.prototype._balanceChildrenForItem = function (extraGravities, grandChildren, initialParentOrgItem) {
    var graph, /* relations graph */ graphGravities,
        index, len,
        index2, len2,
        index3, len3,
        index4, len4,
        ...
        fromItemId, toItemId,
        itemId, startItemId,
        levelKey, levelExtraGravities,
        maxConnections, connections, count, sequence;
            ...
            if (this._orgItemChildren.hasOwnProperty(parentOrgItem.id)) {
                ...
                if (children.length > 0) {
                                ...
                                if (childExtraGravities.hasOwnProperty(levelKey)) {
                                    ...
                                    for (index2 = 0, len2 = levelExtraGravities.length; index2 < len2; index2 += 1) {
                                        ...
                                        if (extraGravity.commonParent == parentOrgItem.id) {
                                            ...
                                            if (this._orgItems[extraGravity.fromParent].childIndex < this._orgItems[extraGravity.toParent].childIndex) {
                                                toItemId = rightId;
                                            } else {
                                                toItemId = leftId;
                                            }
                                        ...
                                        if (childOrgItem.id != toItemId) {
                                            ...
                                            if (!graph[childOrgItem.id].hasOwnProperty(toItemId)) {
                                                graph[childOrgItem.id][toItemId] = 0;
                                            }
                                            graph[childOrgItem.id][toItemId] += 1.0;
                    ...
                    if (firstOrgItem != null) {
                        ...
                        /* find maximum */
                        maxConnections = 0;
                        startItemId = firstOrgItem.id;
                        for (fromItemId in graph) {
                            if (graph.hasOwnProperty(fromItemId)) {
                                connections = graph[fromItemId];

                                count = 0;
                                for (toItemId in connections) {
                                    if (connections.hasOwnProperty(toItemId)) {
                                        count += connections[toItemId];
                                    }
                                }
                        ...
                        sequence = primitives.common.getGraphGrowthSequence(graph, startItemId, function (a) {
                            return a;
                        }); //ignore jslint
                        ...
                        for (index3 = 0, len3 = subChildren.length; index3 < len3; index3 += 1) {
                            subOrgItem = this._orgItems[subChildren[index3]];
                    ...
                    if (this._debug) {
                        if (!primitives.common.compareArrays(this._orgItemChildren[parentOrgItem.id], sortedChildren, function(item) {
                            return item.id;
                        })) { //ignore jslint
...
primitives.famdiagram.Controller.prototype._addExtraGravitiesForConnection = function (extraGravities, fromItem, toItem) {
    ...
    while (parentFromItem.parent != parentToItem.parent) {
        if (this._debug) {
            if (parentFromItem.parent == null || parentToItem.parent == null) {
                throw "No common root found. Most likely items belong to different levels!";
            }
        }
...
primitives.famdiagram.Controller.prototype._sortItemsBylevels = function (famItems) {
    var usedEdgesHash = {},
        itemid, famItem;
    /* Distribute this._famItems by levels. */
    for (itemid in famItems) {
        if (famItems.hasOwnProperty(itemid)) {
            famItem = famItems[itemid];
            if (famItem.level == null) {
                famItem.level = 0;
                this._sortItemBylevels(famItem, famItems, usedEdgesHash);
            }
        }
    }
};

primitives.famdiagram.Controller.prototype._sortItemBylevels = function (placedItem, famItems, usedEdgesHash) {
    var index, len, parentItem, childItem,
        tempParentEdges,
        tempChildrenEdges,
        parentEdges = this._getParentEdges(placedItem, usedEdgesHash),
        childrenEdges = this._getChildrenEdges(placedItem, usedEdgesHash);
    ...
    while (parentEdges.length > 0 || childrenEdges.length > 0) {
        ...
        for (index = 0, len = childrenEdges.length; index < len; index += 1) {
            parentItem = famItems[childrenEdges[index].parent];
            childItem = famItems[childrenEdges[index].child];
            ...
            if (childItem.level == null) {
                ...
                childItem.level = this._getMaximumLevel(childItem.logicalParents, famItems) + 1;

                tempParentEdges = tempParentEdges.concat(this._getParentEdges(childItem, usedEdgesHash));
                tempChildrenEdges = tempChildrenEdges.concat(this._getChildrenEdges(childItem, usedEdgesHash));
            } else if (childItem.level <= parentItem.level) {
                /* child item placed and it needs to be shifted below placedItem*/
                this._shiftChartItem(childItem, parentItem.level - childItem.level + 1, famItems);
            }
        }
        /* itterate parents in second turn */
        for (index = 0, len = parentEdges.length; index < len; index += 1) {
            parentItem = famItems[parentEdges[index].parent];
            childItem = famItems[parentEdges[index].child];
            ...
            if (parentItem.level == null) {
                ...
                parentItem.level = this._getMinimumLevel(parentItem.logicalChildren, famItems) - 1;

                tempParentEdges = tempParentEdges.concat(this._getParentEdges(parentItem, usedEdgesHash));
                tempChildrenEdges = tempChildrenEdges.concat(this._getChildrenEdges(parentItem, usedEdgesHash));
            } else if (childItem.level <= parentItem.level) {
                /* child item placed and it needs to be shifted below placedItem*/
                this._shiftChartItem(childItem, (parentItem.level - childItem.level + 1), famItems);
            }
...
primitives.famdiagram.Controller.prototype._getParentEdges = function (placedItem, usedEdgesHash) {
    ...
    for (index = 0, len = placedItem.logicalParents.length; index < len; index += 1) {
        ...
        edgeKey = parentId + ',' + placedItem.id;
        if (!usedEdgesHash.hasOwnProperty(edgeKey)) {
            result.push(new primitives.famdiagram.EdgeItem(parentId, placedItem.id));
            usedEdgesHash[edgeKey] = true;
        }
    }
    return result;
};

primitives.famdiagram.Controller.prototype._getChildrenEdges = function (placedItem, usedEdgesHash) {
...
primitives.famdiagram.Controller.prototype._shiftChartItem = function (shiftedItem, offset, famItems) {
    var index, len, chartItem;
    shiftedItem.level += offset;

    if (offset > 0) {
        for (index = 0, len = shiftedItem.logicalChildren.length; index < len; index += 1) {
            chartItem = famItems[shiftedItem.logicalChildren[index]];
            if (chartItem.level != null) {
                if (chartItem.level <= shiftedItem.level) {
                    this._shiftChartItem(chartItem, shiftedItem.level - chartItem.level + 1, famItems);
                }
            }
        }
    } else {
        for (index = 0, len = shiftedItem.logicalParents.length; index < len; index += 1) {
            chartItem = famItems[shiftedItem.logicalParents[index]];
            if (chartItem.level != null) {
                if (chartItem.level >= shiftedItem.level) {
                    this._shiftChartItem(chartItem, shiftedItem.level - chartItem.level - 1, famItems);
                }
...
primitives.famdiagram.Controller.prototype._getMaximumLevel = function (items, famItems) {
    var index, len, result, chartItem;
    for (index = 0, len = items.length; index < len; index += 1) {
        chartItem = famItems[items[index]];
        if (chartItem.level != null) {
...
primitives.famdiagram.Controller.prototype._getMinimumLevel = function (items, famItems) {
    var index, len, result, chartItem;
    for (index = 0, len = items.length; index < len; index += 1) {
        chartItem = famItems[items[index]];
        if (chartItem.level != null) {
...
primitives.famdiagram.Controller.prototype._sortItemsBylevels2 = function (famItems) {
    var nodes = {},
        key, sortedNodes,
        index, len, node, famItem,
        childIndex, childLen, 
        childItem, childFamItem,
        level;
    for (key in famItems) {
        if (famItems.hasOwnProperty(key)) {
            famItem = famItems[key];
            nodes[key] = famItem.logicalChildren;
        }
    }
    sortedNodes = primitives.common.topologicalSort(nodes);

    /* assign levels*/
    for (index = 0, len = sortedNodes.length; index < len; index += 1) {
        node = sortedNodes[index];

        famItem = famItems[node];
        if (famItem.level == null) {
            famItem.level = 0;
        }
        for (childIndex = 0, childLen = famItem.logicalChildren.length; childIndex < childLen; childIndex += 1) {
            childItem = famItem.logicalChildren[childIndex];
            childFamItem = famItems[childItem];

            childFamItem.level = childFamItem.level == null ? famItem.level + 1 : Math.max(childFamItem.level, famItem.level + 1);
        }
    }

    /* shift items close to their children */
    for (index = sortedNodes.length - 1; index >= 0; index -= 1) {
        node = sortedNodes[index];
...
primitives.famdiagram.Controller.prototype._resortItemsBylevels = function (famItems) {
    var itemsAtLevels = [],
        minimumLevel = null,
        maximumLevel = null,
        nodes = {},
        key, currentLevel, index, itemsAtLevel,
        famItem, sortedNodes, node, level,
        childIndex, childLen, childItem, childFamItem;
    for (key in famItems) {
        if (famItems.hasOwnProperty(key)) {
            famItem = famItems[key];
            ...
            if (famItem.originalLevel != null) {
                ...
                itemsAtLevels[famItem.originalLevel][famItem.id] = famItem;
    ...
    for (index = minimumLevel; index <= maximumLevel; index += 1) {
        ...
        currentLevel = this._setLevelsForItems(itemsAtLevel, famItems, currentLevel, index + 1);
    }
...
primitives.famdiagram.Controller.prototype._setLevelsForItems = function (items, famItems, level, nextOriginalLevel) {
    var result = level,
        ...
        for (key in items) {
            if (items.hasOwnProperty(key)) {
                ...
                for (childIndex = 0, childLen = famItem.logicalChildren.length; childIndex < childLen; childIndex += 1) {
                    ...
                    if (childFamItem.originalLevel == null) {
                        childFamItem.level = childFamItem.level == null ? famItem.level + 1 : Math.max(childFamItem.level, famItem.level + 1);
...
primitives.famdiagram.Controller.prototype._fillInItems = function (famItems, createFamItem) {
    var result = [],
        key, famItem, extNeeded, newFamItem, childItem,
        newLogicalChildren, extLogicalChildren,
        index2, len2, index;
    ...
    for (key in famItems) {
        if (famItems.hasOwnProperty(key)) {
            ...
            while (extNeeded) {
                ...
                for (index2 = 0, len2 = famItem.logicalChildren.length; index2 < len2; index2 += 1) {
                    childItem = famItems[famItem.logicalChildren[index2]];

                    if (famItem.level + 1 < childItem.level) {
                        extLogicalChildren.push(childItem.id);
                    } else {
                        newLogicalChildren.push(childItem.id);
                    }
                }

                if (extLogicalChildren.length > 0) {
                    newFamItem = createFamItem();
                    newFamItem.level = famItem.level + 1;
                    newFamItem.logicalChildren = extLogicalChildren;

                    newLogicalChildren.push(newFamItem.id);
                    famItem.logicalChildren = newLogicalChildren;

                    extNeeded = true;

                    famItem = newFamItem;

                    result.push(newFamItem);
                }
            }
        }
    }
    for (index = 0; index < result.length; index += 1) {
        newFamItem = result[index];
        famItems[newFamItem.id] = newFamItem;
    }
};

primitives.famdiagram.Controller.prototype._setLogicalParents = function (famItems) {
    var key, childIndex, childLen, famItem, childItem;
    for (key in famItems) {
        if (famItems.hasOwnProperty(key)) {
            famItem = famItems[key];
            for (childIndex = 0, childLen = famItem.logicalChildren.length; childIndex < childLen; childIndex += 1) {
                childItem = famItem.logicalChildren[childIndex];
                famItems[childItem].logicalParents.push(key);
            }
        }
    }
};

primitives.famdiagram.Controller.prototype._eliminateManyToMany = function (famItems, createFamItem) {
    var result = [],
        logicalParents = {},
        key, childIndex, childLen, childItem,
        index, newFamItem, famItem,
        newLogicalChildren;
    for (key in famItems) {
        if (famItems.hasOwnProperty(key)) {
            famItem = famItems[key];
            for (childIndex = 0, childLen = famItem.logicalChildren.length; childIndex < childLen; childIndex += 1) {
                childItem = famItem.logicalChildren[childIndex];
                if (!logicalParents.hasOwnProperty(childItem)) {
                    logicalParents[childItem] = [];
                }
                logicalParents[childItem].push(key);
            }
        }
    }
    ...
    for (key in famItems) {
        if (famItems.hasOwnProperty(key)) {
            famItem = famItems[key];
            if (famItem.logicalChildren.length > 1) {
                newLogicalChildren = [];
                for (childIndex = 0, childLen = famItem.logicalChildren.length; childIndex < childLen; childIndex += 1) {
                    childItem = famItem.logicalChildren[childIndex];
                    if (logicalParents[childItem].length > 1) {
                        newFamItem = createFamItem();
                        newFamItem.logicalChildren = [childItem];

                        newLogicalChildren.push(newFamItem.id);

                        result.push(newFamItem);
                    } else {
                        newLogicalChildren.push(childItem);
                    }
                }
                famItem.logicalChildren = newLogicalChildren;
            }
...
primitives.famdiagram.Controller.prototype._showCursorNeighbours = function () {
    var index, len,
        index2, len2,
		treeItem,
		logicalParentItem,
		selectionPathTreeItem, selectionPathTreeItems,
        famItem, items;
    if (this._cursorTreeItem !== null) {
        ...
        /* select all children */
        if (this._cursorTreeItem.itemConfig != null) {
            famItem = this._famItems[this._cursorTreeItem.itemConfig.id];

            items = [];

            items = famItem.children.concat(famItem.parents);

            for (index = 0; index < famItem.children.length; index += 1) {
                items = items.concat(this._famItems[famItem.children[index]].parents);
            }

            for (index = 0; index < famItem.parents.length; index += 1) {
                items = items.concat(this._famItems[famItem.parents[index]].children);
            }

            for (index = 0; index < items.length; index += 1) {
                treeItem = this._treeItemsByUserId[items[index]];
                if (treeItem.visibility === 0/*primitives.common.Visibility.Auto*/) {
                    treeItem.visibility = 1/*primitives.common.Visibility.Normal*/;
                }
            }
...
primitives.orgdiagram.Controller.prototype._defineLogicalParent = function (logicalParentItem, treeItem) {
    var logicalParents = {}, logicalParent,
        parents = {},
        parent,
        newParents = {},
        itemParents, parentKey,
        index, len;
    ...
    */
    if (logicalParentItem.visibility === 4/*primitives.common.Visibility.Invisible*/) {
        for (index = 0; index < logicalParentItem.logicalParents.length; index += 1) {
            logicalParent = logicalParentItem.logicalParents[index];
            parents[logicalParent] = true;
        }
        while (!primitives.common.isEmptyObject(parents)) {
            for (parentKey in parents) {
                if (parents.hasOwnProperty(parentKey)) {
                    parent = this._treeItems[parentKey];
                    if (parent.visibility === 4/*primitives.common.Visibility.Invisible*/) {
                        itemParents = parent.logicalParents;
                        for (index = 0; index < itemParents.length; index += 1) {
                            logicalParent = itemParents[index];
                            newParents[logicalParent] = true;
                        }
                    } else {
                        logicalParents[parent.id] = parent;
                    }
                }
            }
            parents = newParents;
    ...
    if (primitives.common.isEmptyObject(logicalParents)) {
        logicalParents[logicalParentItem.id] = logicalParentItem;
    }

    for (parentKey in logicalParents) {
        if (logicalParents.hasOwnProperty(parentKey)) {
            parent = logicalParents[parentKey];
            if (this._debug && primitives.common.indexOf(parent.logicalChildren, treeItem, function (a, b) { return a.id == b.id; }) > 0) { //ignore jslint
                throw "Duplicate value in logical logicalChildren array.";
            }
            parent.logicalChildren.push(treeItem);
            if (this._debug && primitives.common.indexOf(treeItem.logicalParents, parent.id) > 0) { //ignore jslint
                throw "Duplicate value in logical parents array.";
            }
            treeItem.logicalParents.push(parentKey);
        }
    }
};

primitives.orgdiagram.Controller.prototype._showCursorNeighbours = function () {
    var index, len,
        index2, len2,
		treeItem,
		logicalParentItem,
